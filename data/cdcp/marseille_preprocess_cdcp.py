...
if __name__ == '__main__':
    ...
    cdcp_df_dict = {
        ...
        'ac_rel_pairs': train_cdcp_df_dict['ac_rel_pairs'] + test_cdcp_df_dict['ac_rel_pairs'],
        'ac_rel_types': train_cdcp_df_dict['ac_rel_types'] + test_cdcp_df_dict['ac_rel_types'], 
        'dataset_type': train_cdcp_df_dict['dataset_type'] + test_cdcp_df_dict['dataset_type'],
        'para_types': train_cdcp_df_dict['para_types'] + test_cdcp_df_dict['para_types']
    }

    cdcp_df = pd.DataFrame(cdcp_df_dict)
    cdcp_df.to_csv('./data/cdcp_data_df.csv', index_label="para_id")
    tmp_df = pd.read_csv('./Data/cdcp_data_df.csv')
    with open('./data/CDCP4ELMo.tsv', 'w') as fp:
        for text in cdcp_df['para_text']:
            fp.write(text + '\n')
#     all_ac_types = []
#     for ac_types in test_cdcp_df_dict["ac_types"]:
#         all_ac_types += ac_types
#     all_rel_types = []
#     for ac_rel_types in test_cdcp_df_dict["ac_rel_types"]:
#         all_rel_types += ac_rel_types
    
#     all_rel_pairs = []
#     for ac_rel_pairs in test_cdcp_df_dict["ac_rel_pairs"]:
#         all_rel_pairs += ac_rel_pairs

#     ac_type_stat = Counter(all_ac_types)
#     rel_type_stat = Counter(all_rel_types)
#     print(ac_type_stat)
#     print(rel_type_stat)
#     print(len(all_rel_pairs))
# pass
