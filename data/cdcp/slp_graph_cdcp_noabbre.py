...
for para_text, AC_spans in zip(para_text_list, orig_AC_spans_list):
    ...
    for index, spans in enumerate(AC_spans):
        ...
        if len(sentence_role_list) == 0:
            ...
            sentences_srl_role_matrix_list.append(np.ones([node_num, node_num]))
...
"A complete waste of court resources , an enormous expense to the innocent citizen , and the unlawful detainment and threatening of an innocent citizen and their assets ."
"Identity Theft Tops List for 13th Consecutive Year in Report of National Consumer Complaints"
" From whom to whom along with effective dates and contact info for the creditor and the new owner of the debt ."
" without enforcement Federal laws equate to a bunch of useless words on a paper ."
" Your dialer has been purposely setup that way by YOUR IT dept ."
"AND THIS FREQUENT REPEAT CALLING HAS TO BE STOPPED BY LAW !"
"As I frequent many of the consumer credit forums ,"
" The amount of the investigation is largely lead by the amount of detail the consumer relays ,"
" but most the time the dispute isn ' t that specific ."
" Because it it the consumer ' s phone"
" That ' s credit card crack in my opinion ."
" Alternative 2 doesn ' t work"
" Worse , a judgment doesn ' t guarantee payment on the account ."
