...
from models.pos_map import pair_idx_map, longformer_prefix_ac_map, pair2sequence, pair_num_map
...
def get_eval_result2(res_dict):
    ...
    ARTC_msg3 = 'ARTC-Macro-g: {:.4f}\tSup-g: {:.4f}\tAtc-g: {:.4f}'.format(
        res_dict["ARTC-Macro-g"], res_dict["Sup-g"], res_dict["Atc-g"])
    return ARI_msg, ACTC_msg, ARTC_msg, ARTC_msg2, macro_msg, macro_msg2, ARTC_msg3
...
def actc_error_analysis(all_label_actc, all_pred_actc, para_texts_in):
    false_stat = [0, 0, 0]
    false_samples = {}
    assert len(all_label_actc) == len(para_texts_in), print(len(all_label_actc), len(para_texts_in))
    # print(all_label_actc, all_pred_actc)
    for label, pred, text in zip(all_label_actc, all_pred_actc, para_texts_in):
        err = []
        print(label, pred)
        for la, pr in zip(label, pred):
            if la != pr:
                ...
                err.append([la, pr])
        if len(err) > 0:
            false_samples[f'{text}'] = [label, pred]
    
    print(false_stat)
    print(false_samples)

# def ar_error_analysis(para_texts_in, group_info, all_label_actc2, all_pred_actc2, all_label_ari, all_pred_ari, all_label_artc2, all_pred_artc2):
#     assert len(para_texts_in) == len(all_label_actc2)
...
def ar_error_analysis(para_texts_in, group_info, all_label_actc2, all_pred_actc2, all_label_ari, all_pred_ari, all_label_artc2, all_pred_artc2):
    false_samples = {}
    assert len(para_texts_in) == len(all_label_actc2)
    actc_label = all_label_actc2
    ari_label = []
    ari_pred = []
    idx = 0
    idx2 = 0
    for span_num, group_size in group_info:
        for i in range(group_size):
            # actc_label.append(all_label_actc2[idx: idx+span_num])
            ari_label.append(all_label_ari[idx2: idx2+pair_num_map[span_num]])
            ari_pred.append(all_pred_ari[idx2: idx2+pair_num_map[span_num]])
            idx += span_num
            idx2 += pair_num_map[span_num]
    ac_label2name = {0: "MajorClaim", 1: "Claim", 2: "Premise"}
    ar_label2name = {0: "Support", 1: "Attack"}
    ...
    # print(group_info, actc_label, ari_label, ari_pred, all_label_artc2, all_pred_artc2)
    for actc_l, actc_p, ari_l, ari_p, artc_l, artc_p, text in zip(actc_label,all_pred_actc2, ari_label, ari_pred, all_label_artc2, all_pred_artc2, para_texts_in):  
        for ari_la, ari_pr in zip(ari_l, ari_p):
            if ari_la != ari_pr:
                false_samples[f'{text}'] = [actc_l, actc_p, artc_l, artc_p]
        
        artc_checked = [0 for i in range(len(artc_l))]
        for artc_tuple in artc_p:
            if artc_tuple in artc_l:
                id = artc_l.index(artc_tuple)
                artc_checked[id] = 1
            elif (artc_tuple[1], artc_tuple[0], artc_tuple[2]) in artc_l:
                id = artc_l.index((artc_tuple[1], artc_tuple[0], artc_tuple[2]))
                artc_checked[id] = 1
            # else:
            #     find = 0
            #     for artc_tuple_l in artc_l:
            #         if artc_tuple[0] == artc_tuple_l[0] and artc_tuple[1] == artc_tuple_l[1]:
            #             stat[f'{ac_label2name[actc_l[artc_tuple_l[0]]]}-{ac_label2name[actc_l[artc_tuple_l[1]]]}-{ar_label2name[artc_tuple_l[2]]}'] += 1
            #             find = 1
            #             break
            #         elif artc_tuple[1] == artc_tuple_l[0] and artc_tuple[0] == artc_tuple_l[1]:
            #             stat[f'{ac_label2name[actc_l[artc_tuple_l[0]]]}-{ac_label2name[actc_l[artc_tuple_l[1]]]}-{ar_label2name[artc_tuple_l[2]]}'] += 1
            #             find = 1
            #             break
            #     if find == 0:
            #         stat[f'{ac_label2name[actc_l[artc_tuple_l[0]]]}-{ac_label2name[actc_l[artc_tuple_l[1]]]}-f'] += 1

        if len(artc_l) != 0 and sum(artc_checked) != len(artc_l):
            false_samples[f'{text}'] = [actc_l, actc_p, artc_l, artc_p]

            # for u, checked in enumerate(artc_checked):
            #     if checked == 0:
            #         stat[f'{ac_label2name[actc_l[artc_l[u][0]]]}-{ac_label2name[actc_l[artc_l[u][1]]]}-{ar_label2name[artc_l[u][2]]}'] += 1
    print(false_samples)
