...
from models.promptmodel_bart7_1 import E2EModel
...
from utils.basic_utils import load_json, get_index_positions, flat_list_of_lists, get_edge_frompairs, \
	get_eval_result, _pair2sequence, _sequence2pair, get_tuple_frompairs, args_metric, eval_edge, chop_large_dis
...
def validate(model, val_loader, cfg, train_global_step, mode='dev', model_saver=None):
	...
	st = time.time()
	debug_step = 5
	global best_macro, best_val_macro, best_v2macro, flag, best_macro_g
	...
	# eval_res["Total-Macro-g"] = (eval_res["ARI-Macro"] + eval_res["ACTC-Macro"] + eval_res["ARTC-Macro-g"]) / 3

	if mode == "test" and eval_res["Total-Macro"] > best_macro:
		best_macro = eval_res["Total-Macro"]
		# model_saver.save(0, model)
	...
	TB_LOGGER.add_scalar(f'{mode}/ARI-Macro', eval_res["ARI-Macro"], train_global_step)
	TB_LOGGER.add_scalar(f'{mode}/ACTC-Macro', eval_res['ACTC-Macro'], train_global_step)
	TB_LOGGER.add_scalar(f'{mode}/ARTC-Macro', eval_res["ARTC-Macro"], train_global_step)
